...
val file = io.Source.fromResource("day22.input")
lazy val secrets = file.getLines.toList.map(_.toLong)
...
def prices(secret: Long) = Iterator
  .iterate(secret)(next)
  .map[Int](p => (p % 10).toInt)

def deltas(secret: Long) = prices(secret)
  .sliding(2)
  .map:
    case Seq(a, b) => b - a

def index(secret: Long): Map[(Int, Int, Int, Int), Int] =
  val deltaTups = deltas(secret)
    .take(2000)
    .sliding(4)
    .map:
