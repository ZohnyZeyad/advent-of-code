...
object Interval:
  def apply[N: Integral](n1: N, n2: N): Interval[N] =
    new Interval[N](n1 min n2, n1 max n2)
  def apply(range: Range): Interval[Int] =
    new Interval[Int](range.min, range.max)
...
case class Area(xRange: Range, yRange: Range):
  ...
  def diff(a: Area): List[Area] =
    intersect(a).fold(List(this)): a =>
      for
        xInterval <- Interval(a.xRange) :: Interval(xRange).diff(
          Interval(a.xRange)
        )
        yInterval <- Interval(a.yRange) :: Interval(yRange).diff(
          Interval(a.yRange)
        )
