...
val input = Source.fromFile("day17.input").getLines.toList
...
case class CPU(a: Long, b: Long, c: Long, ip: Int, program: Vector[Int]):
  ...
  def run: List[Int] =
    Iterator
      .unfold[Option[Int], CPU](this): cpu =>
...
object CPU:
  def init(
      ...
      c: Long = 0,
      ip: Int = 0,
      program: Vector[Int] = Vector.empty
  ) =
...
val anss = startState.program.tails.toList.reverse.tail
  .foldLeft(List(0L)): (possiblePrefixes, expected) =>
...
CPU.init(a = 2024).load(0, 1, 5, 4, 3, 0).run == List(4, 2, 5, 6, 7, 7, 7, 7, 3,
  1, 0)
