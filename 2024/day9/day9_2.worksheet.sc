...
def parseDiskMap(diskMap: String): Vector[(Int, Boolean)] = {
  diskMap.zipWithIndex.map { case (char, index) =>
    (char.asDigit, index % 2 == 0)
  }.toVector
}

def createDiskRepresentation(segments: Vector[(Int, Boolean)]): Vector[Char] = {
  segments.zipWithIndex.flatMap { case ((length, isFile), fileId) =>
    if (isFile) Vector.fill(length)(((fileId / 2) + '0').toChar)
...
def compactDisk(disk: Vector[Char]): Vector[Char] = {
  ...
  def moveBlocks(
    ...
    if (leftmostFree >= rightmostFile) disk
    else {
