...
val file = Source.fromFile("./day2.input")
...
def isIncreasing(report: List[Int], acc: List[Int], count: Int): Boolean =
  report match
    case r1 :: r2 :: rest =>
      if (1 <= (r2 - r1) && (r2 - r1) <= 3) then
        isIncreasing(report.tail, acc :+ r1, count)
      else
        isSafe((acc :+ r1)::: rest, count + 1) || isSafe((acc :+ r2)::: rest, count + 1)
    case _ => true
...
def isDecreasing(report: List[Int], acc: List[Int], count: Int): Boolean =
  report match
    case r1 :: r2 :: rest =>
      if ((r2 - r1) <= -1 && -3 <= (r2 - r1)) then
        isDecreasing(report.tail, acc :+ r1, count)
      else
        isSafe((acc :+ r1)::: rest, count + 1) || isSafe((acc :+ r2)::: rest, count + 1)
    case _ => true
