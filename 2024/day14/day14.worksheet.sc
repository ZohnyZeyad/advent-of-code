...
def calculatePosition(
  ...
  val normalizedX = if (newX < 0) newX + width else newX
  val normalizedY = if (newY < 0) newY + height else newY
...
def assignQuadrant(
    p: Point,
    quadrants: Map[Long, (Range, Range)]
): Option[Long] = {
  quadrants
