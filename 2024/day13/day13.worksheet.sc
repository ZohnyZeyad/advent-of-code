...
val file = Source.fromFile("day13.input")
...
val aButtons = lines.collect { case s"Button A: X+$x, Y+$y" =>
  Point(x.toLong, y.toLong)
}

val bButtons = lines.collect { case s"Button B: X+$x, Y+$y" =>
  Point(x.toLong, y.toLong)
}

val prizes = lines.collect { case s"Prize: X=$x, Y=$y" =>
  Point(x.toLong, y.toLong)
}
